...
import PopMessage from "components/Poppers/PopMessage";
...
const TopNav = () => {
	...
	// tooltip control
	const [tooltipCtrl, setTooltipCtrl] = useState({ message: false, notification: false, profile: false });
				...
				<Container fluid="lg" className="px-3 py-3">
					<Row className="align-items-center">
						...
						<Col xs="auto">
								...
								{(popupState) => (
									<>
										<Tooltip
											title="Messages"
											aria-label="messages"
											open={popupState.isOpen ? false : tooltipCtrl.message || false}
											disableHoverListener
											onMouseEnter={() => setTooltipCtrl({ message: true })}
											onMouseLeave={() => setTooltipCtrl({ message: false })}
											TransitionComponent={Zoom}
											classes={{ tooltip: classes.tooltip }}
										>
											<Button {...bindToggle(popupState)} color="inherit" className={`me-2 bg-primary-light ${popupState.isOpen ? "text-primary" : ""}`} classes={{ root: classes.root }}>
								...
								{(popupState) => (
									<>
										<Tooltip
											...
											open={popupState.isOpen ? false : tooltipCtrl.notification || false}
											disableHoverListener
											onMouseEnter={() => setTooltipCtrl({ notification: true })}
											onMouseLeave={() => setTooltipCtrl({ notification: false })}
											TransitionComponent={Zoom}
											classes={{ tooltip: classes.tooltip }}
										>
											<Button
												...
												disabled={activeRoute("/notifications") ? true : false}
											>
												<Badge variant="dot" color="primary" size="small" classes={{ badge: classes.badge }}>
								...
								{(popupState) => (
									<>
										<Tooltip
											...
											open={popupState.isOpen ? false : tooltipCtrl.profile || false}
											disableHoverListener
											onMouseEnter={() => setTooltipCtrl({ profile: true })}
											onMouseLeave={() => setTooltipCtrl({ profile: false })}
											TransitionComponent={Zoom}
											classes={{ tooltip: classes.tooltip }}
										>
											<Button {...bindToggle(popupState)} aria-label="show more" aria-haspopup="true" className="bg-primary-light text-white" classes={{ root: classes.avatarButton }}>
							...
							{/* <Button aria-label="show more" aria-haspopup="true" className="bg-primary-light text-white" classes={{ root: classes.avatarButton }}>
								<Avatar className={classes.smallAvatar} size="small" />
								<i className="fas fa-ellipsis-v ms-2 p-1"></i>
							</Button> */}
						</Col>
